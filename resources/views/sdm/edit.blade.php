...
@section('content')
    <div class="card card-outline card-primary">
        <div class="card-header">
            <h3 class="card-title">Edit Data SDM</h3>
        </div>

        <div class="card-body">
            <form action="{{ url('/sdm/update/' . $sdm->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        ...
                        @if ($sdm->foto_profile)
                            <img src="{{ asset('images/' . $sdm->foto_profile) }}" alt="Foto Profile"
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <select class="form-control" id="role" name="role" required>
                            ...
                            <option value="admin" {{ old('role', $sdm->role) == 'admin' ? 'selected' : '' }}>Admin
                            </option>
                            <option value="dosen" {{ old('role', $sdm->role) == 'dosen' ? 'selected' : '' }}>Dosen
                            </option>
                            <option value="tendik" {{ old('role', $sdm->role) == 'tendik' ? 'selected' : '' }}>Tenaga
                                Pendidik</option>
                        </select>
                        @error('role')
                            <span class="text-danger">{{ $message }}</span>
                ...
                <a href="{{ url('/sdm') }}" class="btn btn-sm btn-secondary mt-3">Cancel</a>
            </form>
        </div>
    </div>
@endsection
